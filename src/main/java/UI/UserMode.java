...
public class UserMode extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        jLabel9.setText(
                "<html> - Providing false information WILL<br> lead to SEVERE repercussions, be warned!</html>");
        jPanel3.add(jLabel9);
        ...
        reveal_moyenne
                .setBorder(javax.swing.BorderFactory.createMatteBorder(1, 1, 1, 1, new java.awt.Color(255, 153, 255)));
        ...
        layout.setHorizontalGroup(
                layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                        .addComponent(jPanel2, javax.swing.GroupLayout.Alignment.TRAILING,
                                javax.swing.GroupLayout.PREFERRED_SIZE, 1243, javax.swing.GroupLayout.PREFERRED_SIZE));
        layout.setVerticalGroup(
                layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                        .addComponent(jPanel2, javax.swing.GroupLayout.PREFERRED_SIZE, 820,
    ...
    public boolean validateData() {
        ...
        if (name.toLowerCase().equals("name")) {
            genHtml += "- Name shouldn't be " + name + "<br>";
        }
        if (surname.toLowerCase().equals("surname")) {
            genHtml += "- Surname shouldn't be " + surname + "<br>";
        }

        // ! To verify that the date is valid
        try {
            formatter.parse(dob, LocalDate::from);
        } // condition
        ...
        if (!isvalidScore(math)) {
            genHtml += "- Math score is not valid <br>";
        }
        if (!isvalidScore(french)) {
            genHtml += "- French score is not valid <br>";
        }
        if (!isvalidScore(geography)) {
            genHtml += "- Geography score is not valid <br>";
        }
        if (!isvalidScore(chemistry)) {
            genHtml += "- Chemistry score is not valid <br>";
        }
        if (!isvalidScore(german)) {
            genHtml += "- German score is not valid <br>";
        }
        if (!isvalidScore(history)) {
            genHtml += "- History score is not valid <br>";
        }
        if (!isvalidScore(english)) {
            genHtml += "- English score is not valid <br>";
        }
        if (!isvalidScore(physics)) {
            genHtml += "- Physics score is not valid <br>";
        }
        if (!isvalidScore(science)) {
            genHtml += "- Science score is not valid <br>";
        }
        if (!isvalidScore(literature)) {
            genHtml += "- Literature score is not valid <br>";
        }
        ...
        if (!(genHtml.equals("<html><h3>INVALID INPUT</h3><br>"))) {
            ...
            Person.ajouteNotes(Double.parseDouble(math), Double.parseDouble(physics), Double.parseDouble(literature),
                    Double.parseDouble(science), Double.parseDouble(chemistry), Double.parseDouble(history),
                    Double.parseDouble(geography), Double.parseDouble(french), Double.parseDouble(english),
                    Double.parseDouble(german));
        } else {
            Person.setHimself(name, surname, dob, Double.parseDouble(math), Double.parseDouble(physics),
                    Double.parseDouble(literature), Double.parseDouble(science), Double.parseDouble(chemistry),
                    Double.parseDouble(history), Double.parseDouble(geography), Double.parseDouble(french),
                    Double.parseDouble(english), Double.parseDouble(german));
        }
        return true;
    }
    ...
    public void Publish_To_DBActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_Publish_To_DBActionPerformed
        ...
        int confirmation = Dialogs.ConfirmDialog("Submission",
                "Are you sure you want to submit the informations provided? this action is IRREVERSIBLE!");
        if (confirmation == JOptionPane.YES_OPTION) {
            ...
            try (PreparedStatement statement = connection.prepareStatement(query)) {
                ...
                statement.setString(index++, dobString);
                ...
                Dialogs.writeErr("Error",
                        "<html>Error inserting data into database: <br>" + ex.getMessage() + "</html>");
            }
        }
